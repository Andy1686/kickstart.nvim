...
local function lsp_attach(args)
  ...
  if lsp_have_feature.rename(client) then
    vim.keymap.set('n', 'grn', lsp_buf_rename_use_priority_or_select, { buffer = bufnr, desc = 'lsp rename' })
  end
  ...
  if lsp_have_feature.inlay_hint(client) then
    ...
    vim.keymap.set('n', '<leader>ti', toggle_inlay_hint, { buffer = bufnr, desc = 'lsp inlay toggle' })
  end
